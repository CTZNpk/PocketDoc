...
exports.generateQuiz=async(req,res)=>{
    ...
    try {
        if(!summaryId || !difficulty ||!numQuestions ){
            res.status(404).json({error:"summaryId,numQuestions,difficulty are required"})
        }
...
exports.userCompletesQuiz=async(req,res)=>{
    ...
    try {
        ...
        if(user){
            ...
            await user.save()
            res.status(200).json({message:"Quiz History Updated Successfully",user})

        }
    } catch (error) {
        res.status(200).json({error:`Error Updating user quiz history: ${error.message}`})
...
exports.getUserQuizHistory=async(req,res)=>{
    ...
    try {
        const user=await userModel.findById(userId).populate("quizHistory.quizId")
        if(!user){
        ...
        res.status(500).json({error:`Error Getting user Quiz History: ${error.message}`})
