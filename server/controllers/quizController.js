...
const generateAndStoreQuiz = async (req, res) => {
  try {
    ...
    const requiredFields = ['file_path', 'answer_formats'];
...
const getUserQuizHistory = async (req, res) => {
  try {
    ...
    if (!quizzes.length) {
      return res.status(404).json({ error: "No quiz history found for this user" });
    }
    ...
    const history = quizzes.map((quiz) => ({
      ...
      submittedAt: quiz.submissions.find((s) => s.userId.toString() === userId)?.submittedAt,
      score: quiz.submissions.find((s) => s.userId.toString() === userId)?.score,
      totalQuestions: quiz.quiz.length,
    }));
